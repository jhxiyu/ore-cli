...
check_and_claim() {
  ...
  #echo "检查 ${keyfile} 的奖励..."
  # 获取当前奖励数额
  reward=$(ore --rpc https://wallet.okex.org/fullnode/sol/discover/rpc --keypair "${keyfile}" rewards | awk '{print $1}')
  ...
  if [ "$reward_int" -ge "$threshold_int" ]; then
    ...
    claim_output=$(ore --rpc https://late-burned-valley.solana-mainnet.quiknode.pro/3008f13829394f22a8aa1724982865a34fdf/ --keypair "${keyfile}" --priority-fee 5000000 claim)
